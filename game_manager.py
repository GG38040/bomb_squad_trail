...
class GameManager:
    def __init__(self, width, height, asset_loader, music_manager, font_manager):
        self.width = width
        self.height = height
        ...
        # Initialize screens
        self.screens = {
            GameState.MENU: MenuScreen(width, height, asset_loader, font_manager, music_manager),
            GameState.TRAVEL: TravelScreen(width, height, asset_loader, font_manager, music_manager),
            GameState.MINIGAME: MinigameScreen(width, height, asset_loader, font_manager, music_manager),
            GameState.OUTCOME: OutcomeScreen(width, height, asset_loader, font_manager, music_manager)
        }
    ...
    def handle_keydown(self, key):
        ...
        if self.current_state == GameState.MENU:
            if key == pygame.K_1:
                ...
                self.game_start_time = pygame.time.get_ticks()  # Start tracking time
            elif key == pygame.K_2:
                self.running = False
    ...
    def update(self):
        ...
        if self.current_state != GameState.OUTCOME and self.game_start_time is not None:
        ...
        if self.current_state == GameState.TRAVEL:
            ...
            if self.timers['travel'] is not None:
                elapsed = (pygame.time.get_ticks() - self.timers['travel']) / 1000
            ...
            if minigame and minigame.game_over:
                if minigame.success:
                    ...
                    if self.game_data['lives'] > 0:
                        ...
                        print(f"Life lost. Lives remaining: {self.game_data['lives']}")
                    
                    if self.game_data['lives'] <= 0:
                        # Game over - freeze final score
                        print(f"Game Over. Final score: {self.game_data['points']}")
